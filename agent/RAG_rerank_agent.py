...
async def rag(state: State): 
    """
    Searches the vector database and reranks the results to answer 
    user questions about medical article in Pubmed.
    """
    question = state["question"]

    # --- 3. Utilisation ---
    results = await compression_retriever.ainvoke(f"{question}")
